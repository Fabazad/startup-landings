...
export const openaiScrapper: Scrapper = async (url: string) => {
  ...
  try {
      ...
      messages: [
        {
          role: 'system',
          content:
            'Tu es un extracteur de données e-commerce. Analyse le contenu Markdown et retourne uniquement un objet JSON contenant: title (string), price (number), imageUrls (array of strings).',
        },
        {
          role: 'user',
    ...
    if (result.title === '' && result.price === 0 && result.imageUrls.length === 0)
