...
- `src/app/api/` — `scrap/` (URL scraping), `blog/list|publish|unpublish|refresh`, `contact/`, `subscriptions/`. `blog/refresh/[slug]` sets `content_refreshed_at` to resurface a post to the top of the list (or `null` to un-resurface); it's the API equivalent of the manual SQL refresh.
